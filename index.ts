const fullText =
  `
2f0cdfe464344e8650edc59daac3504b1710d56b89dce5011e8c90f6`.split("\n");
const m1 = fullText[0];
const m2 = fullText[1];

const m3 = fullText[2];
// const m1 = "280dc9e4";
// const m2 = "3a0a9cab";

function xorHexes(a: Buffer, b: Buffer): Buffer {
  const res: number[] = [];

  for (const [index, el] of a.entries()) {
    res.push(el ^ (b[index] ?? 0));
  }
  return Buffer.from(res);
}

const asciiFullText = fullText.map((line) => Buffer.from(line, "hex"));
// console.log([asci1], "\n", [asci2]);
const pseudoKey = xorHexes(asciiFullText[6], asciiFullText[18]);

// console.log([pseudoKey]);
console.log([
  xorHexes(Buffer.from("With a bare bodkin? Who would "), pseudoKey).toString(),
]);

// 0  For who would bear the wh
// 1  The pangs of dispr
// 2  The insolence of office,
// 3  That patient merit of
// 4  When he himself might
// 5  With a bare bodkin? Who would
// 6  To grunt and sweat und
// 7  But that the dread of
// 8  The undiscovere'd country, fro
// 9  No traveller returns,
// 10 And makes us rather bear
// 11 Than fly to others that w
// 12 Thus conscience doth make
// 13 And thus the n
// 14 Is sicklied o'
// 15 And enterprises of great
// 16 With this regard their cu
// 17 And lose the name of action.